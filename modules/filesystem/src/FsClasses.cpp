...
void BindClasses(Lode::State& vm, Lode::Exports& exports, std::shared_ptr<FsManager> mgr)
{
    ...
    mgr->fileMethods.Set("Read", vm.CreateFastFunction([](Lode::State& vm2, Lode::StackArgs args) {
    ...
    fileNs.Set("Open", vm.CreateFastFunction([mgr](Lode::State& vm2, Lode::StackArgs args) -> Lode::Value {
        ...
        else if (mode == "x") flags = UV_FS_O_WRONLY | UV_FS_O_CREAT | UV_FS_O_EXCL; // fail if exists
        else if (mode == "wx") flags = UV_FS_O_WRONLY | UV_FS_O_CREAT | UV_FS_O_TRUNC | UV_FS_O_EXCL;
        else if (mode == "ax") flags = UV_FS_O_WRONLY | UV_FS_O_CREAT | UV_FS_O_APPEND | UV_FS_O_EXCL;
        else {
    ...
    mgr->readStreamMethods.Set("Start", vm.CreateFastFunction([](Lode::State& vm2, Lode::StackArgs args) {
    ...
    mgr->readStreamMethods.Set("Close", vm.CreateFastFunction([](Lode::State& vm2, Lode::StackArgs) {
        auto stream = Lode::ObjectWrap<lodefs::ReadStream>::Unwrap(vm2, 1);
        if (!stream) { vm2.RaiseError("fs ReadStream:Close: invalid ReadStream"); return Lode::Value(); }
        stream->RequestClose();
        return Lode::Value();
    }));
    rsNs.Set("Create", vm.CreateFastFunction([mgr](Lode::State& vm2, Lode::StackArgs args) -> Lode::Value {
        if (args.Size() < 2 || !args[1].IsString()) {
    ...
    mgr->watcherMethods.Set("Start", vm.CreateFastFunction([](Lode::State& vm2, Lode::StackArgs args) {
    ...
    mgr->watcherMethods.Set("Stop", vm.CreateFastFunction([](Lode::State& vm2, Lode::StackArgs args) {
        auto watcher = Lode::ObjectWrap<lodefs::FileWatcher>::Unwrap(vm2, 1);
        if (!watcher) { vm2.RaiseError("fs Watcher:Stop: invalid FileWatcher"); return Lode::Value(); }
        return watcher->MethodStop(vm2, args);
    }));
    watcherNs.Set("Create", vm.CreateFastFunction([mgr](Lode::State& vm2, Lode::StackArgs args) -> Lode::Value {
        if (args.Size() < 2 || !args[1].IsString()) {

...
struct UdpManager
{
    ...
    void AddSocket(const std::shared_ptr<lodeudp::UdpSocket>& s) { sockets.push_back(s); }
    void Shutdown()
    {
        shuttingDown = true;
        auto copy = sockets;
...
Lode::Table BuildSocketMethods(Lode::State& vm, const std::shared_ptr<UdpManager>& mgr)
{
    ...
    m.Set("Bind", vm.CreateFunction([mgr](Lode::State& vm2, const std::vector<Lode::Value>& args) -> Lode::Value {
    ...
    m.Set("Send", vm.CreateFunction([mgr](Lode::State& vm2, const std::vector<Lode::Value>& args) -> Lode::Value {
        auto self = Lode::ObjectWrap<lodeudp::UdpSocket>::Unwrap(vm2, 1);
        if (!self)
        {
...
LODE_MODULE(vm)
{
    ...
    udpSocketClass.Set("Create", vm.CreateFunction([mgr](Lode::State& vm2, const std::vector<Lode::Value>&) -> Lode::Value {
        if (mgr->shuttingDown)
        {

...
`ci validate` checks package structure and artifacts; it does not replace
dependency installation or lockfile verification. Dependency-bearing CI must
run `lode install --dev --locked` after validation. A missing or stale lockfile
must fail CI rather than being regenerated by the workflow.
